    ...
    public function vote(Request $request)
    {
        ...
        if($voter->projects->count() == 3) {
            ...
            if($projectVoted = $voter->projects()->where('uuid', $request->get('project'))->first()) {
                ...
                $project->voters()->attach($request->get('voter'), ['validate' => 1, 'token' => NULL]);
                ...
                return ['status' => 'vote_success', 'message' => 'Tu voto ha sido registrado. <br>Quieres seguir biendo más proyectos?'];
                //return redirect('/projects/')->with('status' -> 'vote_success', 'message' -> 'Tu voto ha sido registrado. <br>Quieres seguir biendo más proyectos?');
            }
        }
